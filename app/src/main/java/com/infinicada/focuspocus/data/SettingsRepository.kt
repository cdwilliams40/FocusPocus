...
class SettingsRepository(private val prefs: SharedPreferences) {
    ...
    fun setBreakDurationMinutes(minutes: Int) {
        prefs.edit().putInt(Constants.PrefsKeys.BREAK_DURATION_MINUTES, minutes).apply()
    }
    ...
    fun setMaxBreaksPerSession(max: Int) {
        prefs.edit().putInt(Constants.PrefsKeys.MAX_BREAKS_PER_SESSION, max).apply()
    }
    ...
    fun setEmergencyBreakCadenceWeeks(weeks: Int) {
        prefs.edit().putInt(Constants.PrefsKeys.EMERGENCY_BREAK_CADENCE_WEEKS, weeks).apply()
    }
}
