...
/**
 * Every blocking decision the app makes, in one place, driven by a [Context]
 * rather than by any particular Android component.
 *
 * This logic used to live inside [MyAccessibilityService]. It was moved out so
 * that a second foreground detector could drive exactly the same decisions:
 * Play approves `AccessibilityService` use case by case, and an app blocker
 * whose enforcement is welded to that one API has no answer if the declaration
 * is denied or revoked later. Today two detectors exist —
 * [MyAccessibilityService] (window-state events, ~50 ms) and
 * [ForegroundPollingService] (UsageStats polling, 1–2 s) — and both do nothing
 * but call [onForegroundPackage]. New enforcement behaviour belongs here, never
 * in a detector, or the two modes drift apart.
 *
 * Lifecycle: [start] before anything else, [stop] on teardown. One engine per
 * detector; only one detector runs at a time (see
 * [ForegroundPollingService.syncRunState]).
 *
 * Threading: everything runs on the caller's thread, which for both detectors
 * is the main thread. The caches here exist because of that — a blocked open
 * must not pay for a JSON re-parse or a full-day UsageStats scan while the user
 * waits.
 */
class EnforcementEngine(private val context: Context) {
    ...
    /**
     * Begins enforcement: wires the stores, reconciles anything that expired
     * while nothing was running, and subscribes to the minute tick. Idempotent
     * per instance — each detector owns exactly one engine and calls this once.
     */
    fun start() {
        ...
        Log.d(TAG, "Enforcement engine started")
        ...
        try {
            ...
            Log.e(TAG, "Error reconciling state on service start", e)
            reportNonFatal(e)
        }
    ...
    /** Tears down receivers and flushes buffered writes. Safe to call twice. */
    fun stop() {
    ...
    /**
     * The single entry point a foreground detector drives: [packageName] has
     * just taken the foreground. Both detectors — the accessibility service's
     * window-state events and the fallback poller's UsageStats reads — call
     * this and nothing else, which is what keeps the two modes honestly
     * equivalent.
     *
     * Never throws: a detector must not die on one bad package.
     */
    fun onForegroundPackage(packageName: String) {
        try {
            handleForegroundPackage(packageName)
        } catch (e: Exception) {
            Log.e(TAG, "Error handling foreground package", e)
            reportNonFatal(e)
        }
    }
    ...
    private fun isWhitelistBlockable(packageName: String): Boolean =
        whitelistBlockableCache.getOrPut(packageName) {
            AppUtils.isWhitelistBlockable(context, packageName)
        }
