...
import com.infinicada.focuspocus.GuardNotifier
import com.infinicada.focuspocus.MainActivity
import com.infinicada.focuspocus.OverlayActivity
import com.infinicada.focuspocus.PrefsHelper
import com.infinicada.focuspocus.Progression
import com.infinicada.focuspocus.ProgressionMath
import com.infinicada.focuspocus.ProgressionNotifier
import com.infinicada.focuspocus.R
import com.infinicada.focuspocus.SessionManager
import com.infinicada.focuspocus.SessionNotifier
import com.infinicada.focuspocus.TimeLimitChecker
...
import com.infinicada.focuspocus.computeScheduleEndMillis
...
import com.infinicada.focuspocus.shouldDeactivateSchedule
import java.util.Calendar

/**
 * Every blocking decision FocusPocus makes, and every action it takes on one,
 * with no dependency on *how* the foreground app was detected.
 *
 * This used to be the body of [com.infinicada.focuspocus.MyAccessibilityService].
 * It was lifted out so a second detector can drive the same enforcement: the
 * accessibility service feeds it `TYPE_WINDOW_STATE_CHANGED` events, and
 * [FallbackEnforcementService] feeds it UsageStats polling results when
 * accessibility is unavailable — a policy change, Advanced Protection Mode, or
 * an OEM killing the service. Both paths share one code path, so the fallback
 * can never drift into enforcing different rules from the primary.
 *
 * Nothing here needs the accessibility API: apps are closed with a HOME intent
 * and overlays are plain activity starts, both of which any [Context] can do
 * (the fallback needs SYSTEM_ALERT_WINDOW to start them from the background —
 * see [FallbackEnforcementService]).
 *
 * Not thread-safe by design: a driver must call into it from a single thread
 * (both drivers use their main looper), matching the accessibility service's
 * original single-threaded event delivery.
 */
class BlockingEngine(
    private val context: Context,
    /**
     * Called on every minute tick, for a driver that wants to record its own
     * liveness. The engine itself has no opinion on which driver it belongs to —
     * see [com.infinicada.focuspocus.MyAccessibilityService], which uses this to
     * stamp a heartbeat the fallback can notice going stale.
     */
     ...
     * via [com.infinicada.focuspocus.Blocker.autoAddNewApps] — closing the loophole
     * of installing a fresh distraction mid-session.
     */
    private fun autoAddNewAppToBlockers(newPackageName: String) {
        ...
        if (updated != blockers) {
            sharedPreferences.edit()
                ...
                .apply()
            Log.d(TAG, "Auto-added new app to opted-in enchantments")
        }
    }
    ...
    private val timeLimitChecker by lazy { TimeLimitChecker(context) }
        ...
        override fun onReceive(ignored: Context?, intent: Intent?) {
            when (intent?.action) {
                Intent.ACTION_TIME_TICK -> try {
                    ...
                    // The minute tick must never take the driving service down —
                    // blocking, schedule enforcement, and limits all depend on it
                    // staying alive.
                    Log.e(TAG, "Error in minute tick", e)
                    reportNonFatal(e)
                }
    ...
    private fun onMinuteTick() {
        ...
        // Detectors only report app switches, so without this a user could stay
        // inside an app past its daily limit indefinitely.
    ...
    /**
     * Reconciles state that drifted while nothing was enforcing (a reboot, a
     * revoked service, a switch between enforcement modes), then subscribes to the
     * minute tick and package changes. Must be called exactly once per engine, and
     * only one engine may be live at a time — [EnforcementController] is what
     * guarantees that.
     */
    fun start() {
        ...
        // Clean up any break or timed session that expired while nothing was enforcing
        ...
        // prevent the engine from finishing its startup wiring below.
        try {
            enforceTimedSessionExpiry()
            checkMissedScheduleActivation()
            // Pact revisions that came due while nothing was enforcing apply
            // before the suspension sync reads the stores.
            pactRevisionManager.applyDueRevisions()
            DeviceOwnerManager.syncSuspensions(context)
            // Re-assert uninstall protection on every engine start: provisioning
            ...
            Log.e(TAG, "Error reconciling state on engine start", e)
            reportNonFatal(e)
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            context.registerReceiver(timeTickReceiver, filter, Context.RECEIVER_NOT_EXPORTED)
        } else {
    ...
    private fun createNotificationChannel() {
        try {
            val name = context.getString(R.string.rituals_channel_name)
            val descriptionText = context.getString(R.string.rituals_channel_description)
    ...
    private fun checkMissedScheduleActivation() {
        ...
        val schedules: List<Schedule> = try {
            ...
            Log.e(TAG, "Error parsing schedules in checkMissedScheduleActivation", e)
            return
        }
        ...
        for (schedule in schedules) {
            try {
                ...
                if (inWindow) {
                    Log.d(TAG, "Activating missed schedule on service start: ${schedule.name}")
                    activateSchedule(schedule)
                    return
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error checking missed schedule: ${schedule.name}", e)
            }
        }
    }
    ...
    private fun enforceTimedSessionExpiry() {
        ...
        if (sharedPreferences.getBoolean(Constants.PrefsKeys.IS_ON_BREAK, false)) {
            ...
            if (breakEnd in 1..now) {
                ...
                editor.apply()
                Log.d(TAG, "Break expired while UI was away — resuming focus mode")
        ...
        if (focusEnd in 1..now) {
            Log.d(TAG, "Timed session expired while UI was away — stopping session")
    ...
    private fun maybeStartAutoBreak() {
        ...
        DndController.updateDndState(context)
        DeviceOwnerManager.syncSuspensions(context)
        Log.d(TAG, "Auto-break started after ${intervalMinutes}m of focus")
    ...
    private fun sendAutoBreakNotification(focusedMinutes: Int, breakMinutes: Int) {
        ...
        val builder = NotificationCompat.Builder(context, Constants.RITUALS_CHANNEL_ID)
            .setSmallIcon(R.mipmap.fplogo_round)
            .setContentTitle(context.getString(R.string.auto_break_started_title))
            .setContentText(context.getString(R.string.auto_break_started_message, focusedMinutes, breakMinutes))
        ...
        try {
            ...
            Log.e(TAG, "Failed to send auto-break notification", e)
        }
    }
    ...
    private fun checkSchedules() {
        ...
        if (activeScheduleId != null) {
            ...
            if (activeSchedule != null) {
                try {
                    ...
                    if (endParts.size == 2 && startParts.size == 2) {
                        ...
                        if (endHour !in 0..23 || endMinute !in 0..59 || startHour !in 0..23 || startMinute !in 0..59) {
                            Log.e(TAG, "Invalid schedule time: ${activeSchedule.effectiveStartTime}-${activeSchedule.effectiveEndTime}")
                    ...
                    Log.e(TAG, "Error parsing schedule end time", e)
                }
            } else {
                ...
                Log.w(TAG, "Active schedule $activeScheduleId no longer exists, clearing dangling reference")
                ...
                if (schedule.effectiveDays.contains(currentDay)) {
                    ...
                    if (parts.size == 2) {
                        ...
                        if (startHour !in 0..23 || startMinute !in 0..59) {
                            Log.e(TAG, "Invalid schedule start time: ${schedule.effectiveStartTime}")
    ...
    private fun activateSchedule(schedule: Schedule) {
        ...
        if (validNames.isEmpty()) {
            Log.e(TAG, "Schedule '${schedule.name}' references no valid blockers, skipping activation")
            return
        }
        ...
        sendRitualNotification(
            title = context.getString(R.string.ritual_started_title),
            message = context.getString(R.string.ritual_started_message, schedule.name),
            scheduleId = schedule.id,
    ...
    private fun deactivateSchedule(schedule: Schedule) {
        ...
        sendRitualNotification(
            title = context.getString(R.string.ritual_ended_title),
            message = context.getString(R.string.ritual_ended_message, schedule.name),
            scheduleId = schedule.id,
    ...
    private fun sendRitualNotification(title: String, message: String, scheduleId: String, isEndNotification: Boolean) {
        ...
        try {
            ...
            Log.e(TAG, "Failed to send ritual notification", e)
        }
    }

    /**
     * The single input every enforcement decision hangs off: [packageName] just
     * came to the foreground. Callers may report the same package repeatedly —
     * the debounce and the `openTrackingPackage` guard make redundant reports
     * cheap, which is what lets the polling fallback call this every tick
     * without knowing whether anything changed.
     *
     * Never throws: a driver losing its loop to one bad package means nothing is
     * blocked until the user notices, so failures are swallowed and reported.
     */
    fun onForegroundApp(packageName: String) {
        try {
            handleForegroundApp(packageName)
        } catch (e: Exception) {
            Log.e(TAG, "Error handling foreground app $packageName", e)
            reportNonFatal(e)
        }
    }

    /** Ends the foreground app's continuous-use session (screen off, or driver stopping). */
    fun onScreenOff() {
        try {
            // Screen off ends the foreground app's continuous-use session —
            // otherwise idle screen-off time keeps counting toward the session
            // limit and triggers a cooldown for use that never happened.
    ...
    private fun handleForegroundApp(packageName: String) {
        ...
        if (packageName == context.packageName || isLauncher(packageName) || isInputMethod(packageName) || isSystemUI(packageName)) {
            ...
            if (packageName == context.packageName || isLauncher(packageName)) {
                currentForegroundPackage?.let { sessionCooldownManager.onAppLeft(it) }
                currentForegroundPackage = null
            }
            return
        ...
        // enforce time limits while the user stays inside an app (no switch is reported then).
        ...
        if (focusActive && nfcLockMode && packageName in SETTINGS_PACKAGES) {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "NFC Lock: Blocking settings app: $appName")
            lastAppBlockTimes[packageName] = now
            sessionCooldownManager.onAppLeft(packageName)
            currentForegroundPackage = null
            closeApp()
            showOverlay(appName, context.getString(R.string.service_talisman_lock))
            return
        }
        ...
        if (focusActive) {
            ...
            for (blocker in activeBlockers) {
                if (blocker.shouldBlock(packageName) && !isConditionallyUnlocked(blocker.name)) {
                    val appName = AppUtils.getAppName(context, packageName)
                    if (BuildConfig.DEBUG) Log.d(TAG, "Blocking app: $appName")
                    lastAppBlockTimes[packageName] = now
    ...
    private fun checkTimeLimitAndBlock(packageName: String) {
        ...
        if (windowActive && dailyLimit > 0 && timeLimitChecker.shouldBlock(packageName, dailyLimit)) {
            ...
            val appName = AppUtils.getAppName(context, packageName)
            if (BuildConfig.DEBUG) Log.d(TAG, "Time limit exceeded: $appName")
            lastAppBlockTimes[packageName] = now
            sessionCooldownManager.onAppLeft(packageName)
            currentForegroundPackage = null
            recordBlockEvent(packageName, "Time Limit")
            closeApp()
            showOverlay(appName, context.getString(R.string.service_daily_time_limit))
            return
        }
        ...
        if (pactConfig != null && windowActive) {
            ...
            if (isTimeLimitConditionallyUnlocked(packageName)) return
            val appName = AppUtils.getAppName(context, packageName)
            if (BuildConfig.DEBUG) Log.d(TAG, "Pact gate: $appName blocked by default")
            lastAppBlockTimes[packageName] = now
            sessionCooldownManager.onAppLeft(packageName)
            currentForegroundPackage = null
    ...
    private fun isConditionallyUnlocked(blockerName: String): Boolean {
        ...
        return rules.any { rule ->
            ...
            val usedMs = UsageStatsHelper.getPackageUsageToday(context, rule.requiredAppPackage)
    ...
    private fun getCachedTimeLimits(): Map<String, Int> {
        ...
        return try {
            ...
            Log.e(TAG, "Error parsing time limits JSON", e)
            cachedTimeLimitsJson = null
            cachedTimeLimits = emptyMap()
            emptyMap()
        }
    ...
    private fun flushBlockEvents() {
        ...
        val existing: MutableList<BlockEvent> = when {
            ...
            json == cachedBlockEventsJson -> cachedBlockEvents.toMutableList()
                ...
                Log.e(TAG, "Error parsing block events JSON", e)
                mutableListOf()
            }
        }
    ...
    private fun getCachedTimeLimitConfigs(): Map<String, AppTimeLimit> {
        ...
        return try {
            ...
            Log.e(TAG, "Error parsing time limit configs JSON")
            cachedTimeLimitConfigsJson = null
            cachedTimeLimitConfigs = emptyMap()
            emptyMap()
        }
    ...
    private fun checkPacingNotification(packageName: String, limitMinutes: Int) {
        ...
        val usedMinutes = AppTimeLimitManager.getUsedMinutesToday(context, packageName)
    ...
    private fun showPacingToast(
        ...
        val message = when {
            threshold >= 90 -> context.getString(
                R.string.pacing_toast_90, appName, usedMinutes, limitMinutes
            )
            threshold >= 75 -> context.getString(
    ...
    private fun showPactOverlay(appName: String, packageName: String, config: AppTimeLimit) {
        ...
        if (alternative != null && isPackageInstalled(alternative)) {
            ...
            intent.putExtra("pactAlternativeName", AppUtils.getAppName(context, alternative))
        }
        context.startActivity(intent)
    }
    ...
    private fun trackOpenTransition(packageName: String) {
        if (packageName == context.packageName || isInputMethod(packageName) || isSystemUI(packageName)) return
        val now = System.currentTimeMillis()
    ...
    private fun maybeWarnPactEnding(packageName: String) {
        ...
        if (remainingMs in 1..90_000 && pactWarnedExpiries[packageName] != expiry) {
            ...
            val appName = AppUtils.getAppName(context, packageName)
            val message = context.getString(R.string.pact_ending_toast, appName)
            Handler(Looper.getMainLooper()).post {
                Toast.makeText(context, message, Toast.LENGTH_LONG).show()
            }
        }
    ...
    private fun maybeNotifySealsLifted() {
        ...
        (previous - active).forEach { pkg ->
            GuardNotifier.postSealLifted(context, sharedPreferences, pkg)
        }
    }
